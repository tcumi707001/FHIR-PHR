...
*/	
	...
	$authorizationToken = $header['authorization-jwt']; //token內容
	...
	$payload = json_decode(JWT::urlsafeB64Decode($jwt[1]),true);
								...
								for ($j=0;$j<sizeof($payload['scope'][$i]['url']);$j++){
									...
									if($scope_resourcetype == $request_resourcetype){ //比對resourceType
										...
										if($checkOn_parameters == (sizeof($scope_parameters))){ //必須要完成與scope內容相同的授權內容檢查 (date需包含於範圍內)
											echo GetAPI($payload['scope'][$i]['url'][$j]); //檢查完向FHIR-server取得資料response回client
										}
									}
								}
							}
					...
					case 'DELETE':
						...
						for ($i=0;$i<sizeof($payload['scope']);$i++){
							if($payload['scope'][$i]['method'] == 'DELETE'){
								for ($j=0;$j<sizeof($payload['scope'][$i]['url']);$j++){
									$response = json_decode(GetAPI($payload['scope'][$i]['url'][$j]),true); //以DELETE授權內容的URL，向FHIR GET資源
									if($response['id'] == $request_id){ //檢驗資源ID，是否與請求相同
					...
					case 'PUT': //JSON需要指定文件ID
						...
						for ($i=0;$i<sizeof($payload['scope']);$i++){
							if($payload['scope'][$i]['method'] == 'PUT'){
								for ($j=0;$j<sizeof($payload['scope'][$i]['url']);$j++){
									$response = json_decode(GetAPI($payload['scope'][$i]['url'][$j]),true); //以PUT授權內容的URL，向FHIR GET資源
									if($response['id'] == $request_id){ //檢驗資源ID，是否與請求相同
